...
from pyrocko.guts import Any, Bool, Choice, Dict, Float, Int, List, Object, \
    String, StringChoice, Tuple
...
class PreprocessConfig(Object):
    target_fs = Float.T(
        help='Target sampling rate to downsample the trace. Unit: Hz')
    fmin = Float.T(
        help='Lower corner frequency of the bandpass filter. Unit: Hz')
    fmax = Float.T(
        help='Upper corner frequency of the bandpass filter. Unit: Hz')
    ford = Int.T(
        help='Order of the filter. Default: 4', default=4)
    tfade_taper = Float.T(
        help='Rise/fall time of cosine taper applied in time domain at '
             'both ends of a trace. Unit: s')
    pre_filt = Tuple.T(
        n=4, content_t=Float.T(),
    ...
    def check_nyquist(self):
        ...
        intros = ['Lower corner freq. of bandpass',
                  'Higher corner freq. of bandpass']
...
class SpecgramConfig(Object):
    window_tlen = Float.T(
        help='Time length of each window to take FFT. Unit: s')
    overlap = Float.T(
        help='Overlap between windows. Must be between 0 and 1.')
    kaiser_beta = Float.T(
    ...
    after_tp = Float.T(
        help='Offset after P arrival to crop final spectrogram. Unit: s')
    fmin = Float.T(
        help='Lower frequency bin to crop final spectrogram. Unit: Hz')
    fmax = Float.T(
        help='Upper frequency bin to crop final spectrogram. Unit: Hz')
    cut_first = Bool.T(
...
class FeatureExtractDatasetConfig(Object):
    ...
    mseed_fname = String.T(
        help='miniSEED file names template. Must include a *braced* '
             ...
             'in Pyrocko event attribute "extras".', yamlstyle="'")
    ...
    arrivals_fname = String.T(
        help='Dummy-event file names template. Must include a *braced* '
             ...
             'DummyEvent attribute "extras".', yamlstyle="'")
    ...
    def __init__(self, **kwargs):
        ...
        self.specgram_fname_template = Template(
            f'${{{self._extract_template_placeholder()}}}.nc')   # netCDF
    ...
    def check_inpaths(self):
        ...
        """
        paths = (
            self.events_fpath, self.stations_fpath, self.xmlresps_fpath,
            self.mseed_dirpath, self.arrivals_dirpath)
...
class FeatureExtractConfig(Object):
    ...
    def read_from_file(cls, filepath):
        ...
        for path_name in (
                'events_fpath', 'stations_fpath', 'xmlresps_fpath',
                'mseed_dirpath', 'arrivals_dirpath', 'specgram_dirpath'):
            # Resoving data paths
...
class TrainDatasetConfig(Object):
    events_fname = String.T(
        help='Catalog of seismicity. Format: Pyrocko events', yamlstyle="'")
    root_dirname = String.T(
        help='Directory with all netCDF arrays.', yamlstyle="'")
    sample_fname = String.T(
        help='Sample file name template. Must include a *braced* placeholder '
             'of the form ${sample_id}.', yamlstyle="'")
...
class CAEDECTrainParamsConfig(Object):
    ...
    clust_init_method = StringChoice.T(
        choices=['kmeans', 'gmm'], yamlstyle="'")
    pretrained_model = String.T(
        help='Pretrained autoencoder. Must be the file "name" only. It will '
             ...
             'to get accessed.', yamlstyle="'")


class TrainConfig(Object):
    path_prefix = String.T(
        help='All files/directories referenced are treated relative '
             'to the location of the configuration file', yamlstyle="'")
    problem_name = String.T(
        help='Name used to identify the output directory.', yamlstyle="'")
    run_dirname = String.T(
        help='Parent directory where to save the output directory.',
        yamlstyle="'")
