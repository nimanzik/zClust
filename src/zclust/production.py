...
def initialize_cluster_centroid(X, n_clusters, method="kmeans"):
    ...
    if method == "kmeans":
        kmeans = KMeans(
            n_clusters=n_clusters, n_init=100, max_iter=1000, random_state=seed
        ).fit(X)
        return kmeans.cluster_centers_
    else:
        gmm = GaussianMixture(
            n_components=n_clusters, n_init=10, max_iter=1000, random_state=seed
        ).fit(X)
        return gmm.means_
...
def cluster(dec_model, data_loader, device, n_clusters):
    ...
    soft_assignments = torch.empty((n_samples, n_clusters), dtype=torch.float32)
...
def main():
    ...
    train_parser = subparsers.add_parser("train", description="Training Mode")
    encode_parser = subparsers.add_parser("encode", description="Encoding Mode")
    cluster_parser = subparsers.add_parser("cluster", description="Clustering Mode")
    ...
    if hasattr(args, "step") and args.step == "pretrain":
        ...
        model = ConvEncoderDEC(
            n_clusters=pconfig.n_clusters, n_features=pconfig.n_features
        ).to(device)

    # Training mode
    if args.cmd == "train":
        trainer_kwargs = {
            "optimizer": optim.Adam(model.parameters(), lr=pconfig.learning_rate),
        ...
        if args.make_plot:
            ...
            ax.plot(range(1, len(trainer.train_costs) + 1), trainer.train_costs)
            ax.set(xlabel="Number of epoch", ylabel="Average cost")
            fig_fname = save_dirpath.joinpath(
                out_fname_template.format("learning_curve.png")
            ).as_posix()
        ...
        if args.save:
            ...
            model_fname = save_dirpath.joinpath(
                out_fname_template.format("model.pth")
            ).as_posix()
            ...
            costs_fname = save_dirpath.joinpath(
                out_fname_template.format("costs.npy")
            ).as_posix()
            ...
            log_fname = save_dirpath.joinpath(
                out_fname_template.format("log.txt")
            ).as_posix()
            with open(log_fname, "w") as fid:
                ...
                if trainer.elapsed_time < 60:
                    ...
                    fid.write(f"Elapsed time: {trainer.elapsed_time / 60.0:.2f} min")
    ...
    # Encoding or clustering mode
        ...
        model.load_state_dict(torch.load(model_fpath, map_location=device), strict=True)
